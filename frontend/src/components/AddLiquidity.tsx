...
import {
  ...
  FACTORY_ADDRESS,
  GAS_PRICE_GWEI,
  GAS_LIMIT as GLOBAL_GAS_LIMIT,
  GAS_LIMIT_CREATE_PAIR as GLOBAL_GAS_LIMIT_CREATE_PAIR,
...
export default function AddLiquidityPro() {
  ...
  const DEFAULT_B =
    ALL.find(t => t.symbol === 'TSWP') || ALL.find(t => !t.isNative) || ALL[1] || ALL[0]
  ...
  const fetchRevertReason = async (txHash: `0x${string}`, providedData?: `0x${string}`) => {
    try {
      ...
      try {
        console.log('[decoded]', decodeFunctionData({ abi: RouterABI as any, data: dataToDecode }))
      } catch {}
      try {
        ...
        console.error('[eth_call reverted]', e?.message || e)
      }
    } catch (e: any) {
      console.error('[fetchRevertReason]', e)
    }
  }
    ...
    try {
      ...
      if (AisN) {
        ...
        if (!(rc && rc.status === 'success')) {
          try {
            await fetchRevertReason(h, data)
          } catch {}
          throw new Error('addLiquidityETH (A native) reverted')
        }
      } else if (BisN) {
        const token = Aaddr!
        await publicClient.simulateContract({
        ...
        if (!(rc && rc.status === 'success')) {
          ...
          throw new Error('addLiquidityETH (B native) reverted')
        }
      } else {
        const tokenA = Aaddr!,
          tokenB = Baddr!
  ...
  const hasValidInput = Number(amountA) > 0 && Number(amountB) > 0

  
  const needsInput = !hasValidInput
    ...
    <div className={styles.pool}>
      ...
      {isOpen && (
        <div className={styles.modalOverlay} onClick={() => setIsOpen(false)}>
          <div className={styles.backPoolAdd} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.swapContainer}>
              ...
              <span className={styles.h2}>
                Your liquidity fuels swaps – earn rewards while empowering DeFi.
              </span>
                ...
                <div className={styles.inlineCol}>
                  <div className={styles.balanceRow}>
                    <small className={styles.balanceLabel}>
                      <span className={styles.labelAdd}>Balance: </span>
                      <span className={styles.balanceWallet}>
...
/>
              ...
              {reserves ? (
                <small className={styles.infoLiquid}>
                  1 {TA?.symbol} ≈ {priceAB?.toFixed(3) ?? '—'} {TB?.symbol} | 1 {TB?.symbol} ≈ {priceBA?.toFixed(3) ?? '—'} {TA?.symbol}
                </small>
              ) : (
      
                      <small className={styles.infoLiquid}>The pair doesn’t exist yet — first supply will create it.</small>
       ...
       <img src={reverse} alt="Logo" className={styles.logoReverse} />
              ...
              <div className={styles.ligneLabel}>
                ...
                {!TA?.isNative && needApproveA && (
                  <button className={styles.btnApprove} onClick={() => onApprove(TA, TA!.symbol)} disabled={pending}>
                    Approve {TA?.symbol}
                  </button>
                )}
                {!TB?.isNative && needApproveB && (
                  <button className={styles.btnApprove} onClick={() => onApprove(TB, TB!.symbol)} disabled={pending}>
                    Approve {TB?.symbol}
                  </button>
                )}
         ...
         <button
  className={`${styles.btnSupply} ${(!canSupply || pending) ? styles.btnDisabled : ''}`}
  onClick={onSupply}
  disabled={!canSupply || pending}
>
  {pending
    ? 'Supplying…'
